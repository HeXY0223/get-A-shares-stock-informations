    ...
    def fetch(self):
        ...
        try:
            ...
            if len(ann_list) > 0:
                ann_long_list = []
                for each in ann_list:
                    #each.reset_index(inplace=True, drop=False)
                    each1 = pd.melt(
                        each,
                        id_vars=['ts_code', 'end_date'],
                        var_name='data_name',
                        value_name='data_value'
                    )
                    ann_long_list.append(each1)
                ann_long = pd.concat(ann_long_list, ignore_index=True)
                ann_long.rename(columns={'end_date': 'trade_date'}, inplace=True)
            ...
            logger.exception(f"获取{self.ts_codes}的数据时出现问题：{e}")
