...
def get_sw_category(engine, src:str='SW2021'):
    load_dotenv()
    ...
    try:
        ...
        if not inspector.has_table('sw_category'):
            logger.info(f"表格 'sw_category' 不存在，正在自动创建...")
            ...
            logger.info("表格 'sw_category' 已存在，无需创建")
    except Exception as e:
        logger.error(f"get_sw_category出现错误：{e}")

if __name__ == '__main__':
    get_sw_category(engine=easyConnect())